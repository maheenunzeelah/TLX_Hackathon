...
import base64
from app import generateResult
...
async def generateCode(upload_file: UploadFile = File(...),userPrompt:str=Form(...)):
    if userPrompt =='':
        ...
        prompt="Given the detected elements and given image, and improvement suggestions write it's html and inline css code."+userPrompt 
    contents = await upload_file.read()
    image = Image.open(io.BytesIO(contents))
